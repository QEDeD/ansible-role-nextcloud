...
To stop using Redis, clear both `nextcloud_redis_hostname` and `nextcloud_redis_socket_path_host`. Keep the old Redis server and connection available while you first run the playbook with the `adjust-nextcloud-config` tag. This removes Nextcloud's persisted `redis`, `memcache.distributed`, and `memcache.locking` settings in one configuration update. Then rerun the installation to remove the container environment, socket mount, session configuration, and any Redis-only network or systemd dependency which you also removed from your configuration.
